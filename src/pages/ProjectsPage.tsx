...
function SilkProjectCard({ project, index }: { project: any; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="silk-card cursor-pointer group flex flex-col md:flex-row gap-6 p-0 overflow-hidden"
    >
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-border/50">
          <div className="flex flex-wrap gap-2">
            {(project.tags || []).slice(0, 3).map((tag: string) => (
              <span key={tag} className="text-[10px] text-muted-foreground bg-muted px-2 py-1 rounded-md font-medium">
                {tag}
              </span>
            ))}
          </div>
...
function ArchitectProjectCard({ project, index }: { project: any; index: number }) {
  ...
  return (
    <motion.div
      ...
      onClick={() => navigate(`/projects/${project.id}`)}
      className="architect-card cursor-pointer group w-full sm:w-[360px] p-0 overflow-hidden"
    >
    ...
    <div className="relative min-h-screen bg-background transition-colors duration-500">
      ...
      <section className="pb-24 px-6 md:px-16 lg:px-24">
        <div className="max-w-6xl mx-auto">
          ...
          {/* GitHub CTA */}
          <div className={`mt-16 p-10 md:p-16 text-center ${isDark ? 'architect-card' : 'silk-card'}`}>
